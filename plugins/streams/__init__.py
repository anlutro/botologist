...
class Stream:
    ...
    def __eq__(self, other):
        if isinstance(other, self.__class__):
            ...
            raise ValueError(
                "Cannot compare Stream with {type}".format(type=type(other).__name__)
            )
...
class StreamManager:
    ...
    def get_new_online_streams(self):
        ...
        if self._cached_streams.initiated:
            cached_stream_urls = [
                stream.url
                for stream in self._cached_streams.get_all()
                if not stream.is_rebroadcast
            ]
            diff = [
                stream
                for stream in streams
                if stream.url not in cached_stream_urls and not stream.is_rebroadcast
            ]
            log.debug(
                "Cached streams: %d - Online streams: %d - Diff: %d",
                len(cached_stream_urls),
                len(streams),
                len(diff),
            )
...
class StreamsPlugin(botologist.plugin.Plugin):
    def __init__(self, bot, channel):
        if "twitch_auth_token" not in bot.config:
            raise ValueError(
                "Must add twitch_auth_token to config.yml to use stream plugin!"
            )
        ...
        for stream in streams:
            ...
            for user_id, subs in self.streams.subs.items():
                if stream.url in subs:
                    highlights.extend(
                        user.nick
                        for user in self.channel.find_users(identifier=user_id)
