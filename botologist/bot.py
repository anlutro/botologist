...
class Bot:
    ...
    def __init__(self, config):
        protocol_module = "botologist.protocol.{}".format(
    ...
    def _handle_privmsg(self, message):
        ...
        if message.is_private:
            ...
            # later re-use
        message.user.is_admin = message.user.identifier in self.admins or (
            message.channel and message.user.identifier in message.channel.admins
        )
    ...
    def _handle_command(self, message, channel):
        ...
        if cmd_string in channel.commands:
            ...
            matching_commands = [
                cmd for cmd in channel.commands if cmd.startswith(cmd_string)
            ]
            if not matching_commands:
                log.debug(
                    '"%s" did not match any commands in channel %s',
                    cmd_string,
                    channel.name,
                )
                return
                ...
                log.debug(
                    '"%s" matched more than 1 command in channel %s',
                    cmd_string,
                    channel.name,
                )
                return
    ...
    def _start_tick_timer(self):
        self.timer = threading.Timer(
            function=self._wrap_error_handler(self._tick), interval=self.TICK_INTERVAL
        )
        self.timer.start()
